...
--[[function timeSel()
    if GUI.Val("Single") == true  then GUI.Val("Single",false) end
    
end]]

-- ADD OPTIONS TO SELECT ITEM/ITEMS AND RENDER THEM
-- ADD TIME SELECTION RENDERING THAT MAKES SENSE
-- 
...
alreadyThere = false
  ...
  reaper.GetSetProjectInfo( 0, "RENDER_CHANNELS" , channel, true ) --set numebr of channels
    ...
    for i=0, num-1 do
        ...
        ----------------------RACCOGLIERE TUTTI GLI INDICI DELLE TRACCE CON EMPTY ITEMS
...
function timeSel()
    ...
    --reaper.GetSetProjectInfo( 0, "RENDER_TAILMS" , tailLenght, true )
    if tail == 8 then
    ...
    --reaper.ShowConsoleMsg("time sel rendering")
...
function singleFile(renderItems)
    ...
    --reaper.ShowConsoleMsg(renderStart.."     ")
    --reaper.ShowConsoleMsg(renderEnd)
    -------------set render name
    name = GUI.Val("TextboxName")
